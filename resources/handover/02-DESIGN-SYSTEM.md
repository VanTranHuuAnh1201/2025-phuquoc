...
> ⚠️ **Đã lên monorepo (08/2026).** Code này giờ ở `apps/2025-phogroup/`. Design
> system cam→hồng dưới đây **chỉ áp dụng cho `apps/2025-phogroup`**, không áp cho
> sản phẩm booking hotel mới (mẫu mới dùng design token riêng từng theme).

```bash
pnpm --filter @repo/2025-phogroup dev     # next dev --turbopack, cổng 3001
pnpm --filter @repo/2025-phogroup build
pnpm turbo lint
```
