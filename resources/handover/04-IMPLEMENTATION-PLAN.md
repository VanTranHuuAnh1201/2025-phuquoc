...
> ⚠️ **Đã lên monorepo (08/2026).** Mọi đường dẫn `src/...` trong tài liệu này
> tính từ **`apps/2025-phogroup/`**, ví dụ `src/app/lib/pricing.ts` →
> `apps/2025-phogroup/src/app/lib/pricing.ts`. Các file trong bảng công việc là file
> **sẽ tạo**, chưa tồn tại — không phải link hỏng.
>
> Kế hoạch này viết cho sản phẩm tour của Pho Group, **độc lập** với sản phẩm
> Booking Hotel đang dựng ở `apps/2026-thenamduhill`.
...
3. Mỗi phase chạy được `pnpm turbo build` sạch trước khi sang phase kế.
...
**DoD Phase 1:** vào `/vi/tour` → redirect đúng; stepper render 7 bước; summary bar sticky; cart persist qua F5; `pnpm turbo build` sạch.
...
| 6.9 | `pnpm turbo lint` sạch + xoá `eslint-disable no-img-element` (chuyển `next/image`) | M |
...
...
```
- Mỗi phase: `pnpm turbo lint build` sạch trước khi commit.
