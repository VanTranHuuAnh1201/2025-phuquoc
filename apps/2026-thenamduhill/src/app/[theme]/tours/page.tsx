 ...
 *
 * Route đọc slot `Tours` từ registry, không biết mẫu nào có trang này. Mẫu chưa
 * dựng thì rơi về `Home` để URL không trả 404 (luật R5).
 */
...
export async function generateMetadata({ params }: PageProps) {
    ...
    return {
        title: `Combo & Tour đảo — ${data.brand.name}`,
...
export default async function Page({ params, searchParams }: PageProps) {
    ...
    const data = await getProperty()
    const Component = theme.Tours ?? theme.Home
