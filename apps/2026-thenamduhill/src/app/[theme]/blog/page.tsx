...
/** /[theme]/blog — cẩm nang, điểm đến & trải nghiệm. */
...
export async function generateMetadata({ params }: PageProps) {
    ...
    return {
        title: `Cẩm nang — ${data.brand.name}`,
...
export default async function Page({ params, searchParams }: PageProps) {
    ...
    const [data, posts] = await Promise.all([getProperty(), getBlogPosts()])

    const Component = theme.Blog ?? theme.Home

    return (
        <LocaleProvider>
            <SiteOverlay />
            <Component data={data} locale={locale} posts={posts} />
        </LocaleProvider>
    )
}
