...
>
> File này là kiến trúc của **NextGig FE Portal** (portal tuyển dụng gig/shift,
> `api.nextgig.sg`) — một dự án KHÁC. Nó nằm ở đây để tham chiếu ý tưởng thiết kế,
> **không phải** để mô tả repo The Nam Du Hill.
>
> **Stack lệch gần như hoàn toàn:** Vite 6 + React Router v7 + TanStack Query +
> Axios + yarn + nginx ⟷ repo này là Next 15 App Router + pnpm monorepo +
> Supabase, **không có axios**, không có TanStack Query.
>
> **Làm theo tài liệu này là đi LÙI so với luật hiện hành:**
> - Tài liệu dùng `Bearer` token trong `localStorage` → `.claude/rules/backend.md` **BE10**
>   yêu cầu cookie `HttpOnly + Secure + SameSite=Strict`
> - Tài liệu `hardCodePermission(role)` ở client → **BE2**: "TUYỆT ĐỐI KHÔNG tin `role` từ client"
> - Envelope `{data, metadata}` → **BE1**: `{success, data, error}` với message `{vi,en}`
> - **Không nhắc mã 409 lần nào** → **BE4**: 409 là mã chống overbooking
>
